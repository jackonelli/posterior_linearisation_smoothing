...
def main():
    ...
    results_dir = Path.cwd() / "results" / experiment_name
    ...
    rmses_comb = np.zeros((num_mc_samples, num_iter))
    ...
    neeses_comb = np.zeros((num_mc_samples, num_iter))
    ...
    for mc_iter in range(num_mc_samples):
        ...
        #     LsIeks(
        #         motion_model, meas_model, num_iter, ArmijoWolfeLineSearch(cost_fn_eks, dir_der_eks, c_1=c_1, c_2=c_2)
        #     ),
        #     states,
        #     measurements,
        #     prior_mean,
        ...
        ipls_estimates = (deepcopy(ms_ipls), deepcopy(Ps_ipls))
        _, _, _, tmp_rmse, tmp_nees = run_smoothing(
        ...
        rmses_comb[mc_iter, :] = tmp_rmse
        neeses_comb[mc_iter, :] = tmp_nees
    ...
    label_ieks, label_lm_ieks, label_ls_ieks, label_ipls, label_comb, label_lm_ipls, label_ls_ipls = (
        "IEKS",
        "LM-IEKS",
        "LS-IEKS",
        "IPLS",
    ...
    nees_stats = [
        ...
        (neeses_comb, label_comb),
        (neeses_lm_ipls, label_lm_ipls),
        (neeses_ls_ipls, label_ls_ipls),
    ]

    save_stats(results_dir, "RMSE", rmse_stats)
    save_stats(results_dir, "NEES", nees_stats)
    plot_scalar_metric_err_bar(rmse_stats, "RMSE")
    plot_scalar_metric_err_bar(nees_stats, "NEES")


def parse_args():
    parser = argparse.ArgumentParser(description="CT model constant sensors metrics experiment.")
