...
class RangeBearing(MeasModel, Differentiable):
    ...
    def jacobian(self, state, time_step=None):
        ...
        atan_outer_diff = 1 / (1 + (delta_y / delta_x) ** 2)
        atan_inner_dx = -delta_y * 1 / (delta_x) ** 2
        atan_inner_dy = 1 / delta_x
        bearings_diff = np.array([atan_outer_diff * atan_inner_dx, atan_outer_diff * atan_inner_dy])
...
class MultiSensorBearings(MeasModel, Differentiable):
    def __init__(self, sensors, meas_noise):
        ...
        TODO: Extend to more than two sensors
        """
        self.sensors = sensors
        self._meas_noise = meas_noise
